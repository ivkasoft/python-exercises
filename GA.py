...
def visualize_initial_and_final(classroom_width, classroom_length, initial_population, final_population):
    ...
    plt.scatter(*zip(*initial_population), color='red', marker='o', label='Начална позиция на точките за достъп до WiFi')

    for i, (x, y) in enumerate(final_population):
        plt.text(x, y, str(i + 1), color='black', fontsize=8, ha='center', va='center')
    plt.scatter(*zip(*final_population), color='blue', marker='o', label='Финална позиция на точките за достъп до WiFi')

    plt.xlabel('Ширина на учебната зала (м)')  # Corrected typo xlable to xlabel
    plt.ylabel('Дължина на учебната зала (м)')  # Corrected typo ylable to ylabel
    plt.title('Начално и крайно положение на точките за достъп')
    plt.legend()
    plt.show()
...
def visualize_dynamic_changes(classroom_width, classroom_length, population, fitness_scores):
    for generation in range(generations):
        ...
        plt.title(f'Сила на WiFi сигнала и разпределение на точките за достъп (Поколение {generation + 1})')  # Added space after Поколение
        plt.legend()
...
access_point_coordinates = population[-1]
best_coverage_point = best_solution
print("Access Point Coordinates:", access_point_coordinates)
print("Best Coverage Point:", best_coverage_point)